...
class ModusRetenciones(models.Model):
    ...
    items = fields.One2many('retenciones.line', 'name', 'Items')
...
#    @api.one
#    @api.depends('retenciones_line.valor_retencion')
#    def _compute_total_retencion(self):
#        self.total_retencion = sum(line.valor_retencion for line in self.retenciones_line)
#        for record in self:
#            record.total_retencion = sum(line.valor_retencion for line in record.retenciones_line)

class RetencionesLine(models.Model):
    _name = 'retenciones.line'
...
#    item_id = fields.Many2one('modus.retenciones', 'Número de Retención')
    ...
    porcent_retencion = fields.Integer('Porcentaje Retención', required=True)
